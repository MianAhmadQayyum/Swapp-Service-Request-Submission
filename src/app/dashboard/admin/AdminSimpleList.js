...
/**
 * Generic add/delete list for simple admin entities (suppliers, teams, etc.).
 *
 * Props:
 *   items      – array of { id, label } to display
 *   fieldName  – the FormData key used when creating (e.g. "title", "name")
 *   placeholder – input placeholder text
 *   onCreate   – server action: (formData) => { error? }
 *   onDelete   – server action: (id) => { error? }
 */
export default function AdminSimpleList({ items, fieldName, placeholder, onCreate, onDelete }) {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 overflow-hidden">
      <form
        ...
        className="p-4 border-b border-zinc-200 dark:border-zinc-700 flex gap-2"
      >
        <input
          value={value}
